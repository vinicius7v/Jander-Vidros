    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-[2.5rem] w-full max-w-2xl shadow-2xl flex flex-col max-h-[90vh]">
        <div className="flex items-center justify-between px-8 pt-6 pb-4 border-b print:hidden">
          <span className="font-black uppercase italic text-gray-700">Pré-visualização</span>
        ...
        <div id="print-area" className="overflow-y-auto p-8">
          <style>{`@media print { body * { visibility: hidden; } #print-area, #print-area * { visibility: visible; } #print-area { position: fixed; top: 0; left: 0; width: 100%; padding: 32px; } .print\\:hidden { display: none !important; } }`}</style>
          ...
          <div className="flex justify-between items-start mb-8">
            ...
            <div className="text-right">
              ...
              <div className="text-xs text-gray-500 mt-1">{new Date(orc.created_at).toLocaleDateString('pt-BR')}</div>
            </div>
          </div>
          ...
          <div className="bg-gray-50 rounded-2xl p-5 mb-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <p className="font-black text-gray-800">{orc.client_name}</p>
              </div>
              {orc.client_phone && (
                <div>
                  <span className="text-xs text-gray-400 uppercase font-bold">Telefone</span>
                  <p className="font-black text-gray-800">{orc.client_phone}</p>
                </div>
              )}
              {orc.client_address && (
                <div className="col-span-2">
                  <span className="text-xs text-gray-400 uppercase font-bold">Endereço</span>
                  <p className="font-black text-gray-800">{orc.client_address}</p>
                </div>
              )}
            </div>
          </div>

          <h2 className="text-xs font-black uppercase tracking-widest text-gray-400 mb-3">Itens</h2>
            ...
            <tbody>
              {orc.items.map((item, i) => (
                <tr key={i} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                  ...
                  <td className="p-3 text-sm text-right font-black text-indigo-600">R$ {(item.qty * parseFloat(item.unit)).toFixed(2)}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <div className="flex justify-end mb-6">
            <div className="bg-indigo-600 text-white rounded-2xl px-8 py-4 text-right">
              <div className="text-xs uppercase font-black tracking-widest opacity-75">Total</div>
          ...
          <div className="grid grid-cols-2 gap-4">
            {orc.validity && (
              <div className="bg-gray-50 rounded-xl p-4">
                <span className="text-xs font-black uppercase text-gray-400">Validade</span>
          ...
          <div className="mt-10 pt-6 border-t border-gray-200 text-center text-xs text-gray-400">
            Este orçamento foi gerado pelo sistema Jander Vidros.
          </div>
        </div>
      </div>
    </div>
...
function ModalOrcamento({ onSave, onCancel }) {
  ...
  const handleSave = (e) => {
    ...
    onSave({ client_name: clientName, client_phone: clientPhone, client_address: clientAddress, validity, notes, items });
  };
      ...
      <div className="bg-white rounded-[2.5rem] w-full max-w-2xl shadow-2xl flex flex-col max-h-[90vh]">
        ...
        <form onSubmit={handleSave} className="overflow-y-auto px-8 py-6 space-y-5">
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <input className={`${inputClass} col-span-2`} placeholder="Nome do Cliente *" value={clientName} onChange={e => setClientName(e.target.value)} required />
              <input className={inputClass} placeholder="Telefone" value={clientPhone} onChange={e => setClientPhone(e.target.value)} />
              <input className={inputClass} placeholder="Endereço" value={clientAddress} onChange={e => setClientAddress(e.target.value)} />
            </div>
          </div>
          <div>
            <div className="flex items-center justify-between mb-3">
              <p className="text-xs font-black uppercase tracking-widest text-gray-400">Itens</p>
              <button type="button" onClick={addItem} className="text-xs font-black text-indigo-600 hover:text-indigo-800 flex items-center gap-1">
          ...
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={onCancel} className="flex-1 py-4 font-black uppercase text-gray-400 hover:text-gray-600">Cancelar</button>
            <button type="submit" className="flex-[2] py-4 bg-indigo-600 text-white rounded-2xl font-black uppercase shadow-xl hover:bg-indigo-700">Gerar Orçamento</button>
          </div>
        </form>
      </div>
    </div>
...
function Services({ user, onLogout }) {
  ...
  const loadAll = async () => {
    setLoading(true);
    try {
      const [svcs, orcs] = await Promise.all([serviceService.getAll(), orcamentoService.getAll()]);
      setServices(svcs);
      setOrcamentos(orcs);
  ...
  const handleSaveService = async (e) => {
    e.preventDefault();
    try {
      await serviceService.create({ client, description, value: parseFloat(value), date, status: 'Pendente' });
      await loadAll();
  ...
  const toggleStatus = async (id, currentStatus) => {
    ...
    try { await serviceService.updateStatus(id, newStatus); await loadAll(); } catch (e) { alert('Erro'); }
  };

  const deleteService = async (id) => {
    if (!window.confirm('Excluir este serviço?')) return;
    try { await serviceService.delete(id); await loadAll(); } catch (e) { alert('Erro'); }
  };

  const handleSaveOrcamento = async (orc) => {
    try {
      await orcamentoService.create(orc);
      await loadAll();
  ...
  const deleteOrcamento = async (id) => {
    ...
    try { await orcamentoService.delete(id); await loadAll(); } catch (e) { alert('Erro'); }
  };
    ...
    <div className="min-h-screen bg-gray-100 font-sans">
      ...
      <div className="max-w-7xl mx-auto p-6">
        <div className="flex justify-between items-center mb-6">
          ...
          <button onClick={() => tab === 'servicos' ? setIsServiceModalOpen(true) : setIsOrcModalOpen(true)}
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredServices.length > 0 ? filteredServices.map(service => (
                <div key={service.id} className={`bg-white rounded-[2rem] p-6 shadow-xl border-t-[12px] ${service.status === 'Concluído' ? 'border-green-500' : 'border-orange-500'}`}>
                  ...
                  <div className="flex justify-between items-center mb-6 pt-4 border-t border-gray-50">
                    <div className="text-[10px] font-black text-gray-400 uppercase flex items-center gap-1">
                      <Calendar size={14} /> {service.date ? new Date(service.date).toLocaleDateString('pt-BR') : ''}
                    </div>
                    <div className="font-black text-indigo-600 text-lg">R$ {Number(service.value).toFixed(2)}</div>
                  </div>
                  <button onClick={() => toggleStatus(service.id, service.status)}
                    className={`w-full py-4 rounded-2xl font-black uppercase text-xs transition-all flex items-center justify-center gap-2 ${service.status === 'Concluído' ? 'bg-gray-100 text-gray-400 hover:bg-orange-100 hover:text-orange-600' : 'bg-green-600 text-white hover:bg-green-700 shadow-lg'}`}>
                    {service.status === 'Concluído' ? <><Clock size={18} /> Reabrir</> : <><CheckCircle size={18} /> Finalizar</>}
                  </button>
                </div>
              )) : (
        ...
        {tab === 'orcamentos' && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {orcamentos.length === 0 ? (
              ...
              const items = typeof orc.items === 'string' ? JSON.parse(orc.items) : orc.items;
              const total = items.reduce((s, i) => s + i.qty * parseFloat(i.unit), 0);
              return (
                <div key={orc.id} className="bg-white rounded-[2rem] p-6 shadow-xl border-t-[12px] border-indigo-500">
                  <div className="flex justify-between items-start mb-4">
                    <span className="px-3 py-1 rounded-full text-[9px] font-black uppercase bg-indigo-100 text-indigo-600">
                      #{String(orc.id).padStart(5, '0')}
                    </span>
                    <button onClick={() => deleteOrcamento(orc.id)} className="text-gray-300 hover:text-red-500 transition-colors">
                      <Trash2 size={18} />
                    </button>
                  </div>
                  <h3 className="text-xl font-black text-gray-800 uppercase italic leading-none mb-1">{orc.client_name}</h3>
                  {orc.client_phone && <p className="text-xs text-gray-400 font-bold mb-3">{orc.client_phone}</p>}
                  <div className="space-y-1 mb-4">
                    {items.slice(0, 3).map((item, i) => (
                      <p key={i} className="text-sm text-gray-500 truncate">• {item.desc}</p>
                    ))}
                    {items.length > 3 && <p className="text-xs text-gray-400">+{items.length - 3} itens...</p>}
                  </div>
                  <div className="flex justify-between items-center pt-4 border-t border-gray-50 mb-4">
                    <div className="text-[10px] font-black text-gray-400 uppercase flex items-center gap-1">
                      <Calendar size={14} /> {new Date(orc.created_at).toLocaleDateString('pt-BR')}
                    </div>
                    <div className="font-black text-indigo-600 text-lg">R$ {total.toFixed(2)}</div>
                  </div>
                  <button onClick={() => setPrintOrc({ ...orc, items })}
            ...
            <form onSubmit={handleSaveService} className="space-y-4">
              ...
              <div className="flex gap-3 pt-4">
                <button type="button" onClick={() => setIsServiceModalOpen(false)}
                  className="flex-1 py-4 font-black uppercase text-gray-400 hover:text-gray-600">Cancelar</button>
                <button type="submit"
                  className="flex-[2] py-4 bg-indigo-600 text-white rounded-2xl font-black uppercase shadow-xl hover:bg-indigo-700">Salvar</button>
              </div>
            </form>
          </div>
        </div>
